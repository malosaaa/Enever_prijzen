    ...
    async def _async_update_data(self):
        ...
        if self.limit_reached_month is not None:
            if self.limit_reached_month == current_month:
                ...
                _LOGGER.debug(
                    "API limiet deze maand bereikt. Update genegeerd, we gebruiken de cache."
                )
                return self.last_data
            else:
                # Het is een nieuwe maand! Word wakker!
                _LOGGER.info(
                    "Nieuwe maand gestart! Enever integratie ontwaakt uit winterslaap."
                )
                self.limit_reached_month = None
                persistent_notification.async_dismiss(self.hass, "enever_api_limit")

        # --- NORMALE START ---
        if self._is_first_run and (
            self.last_data.get("stroom") or self.last_data.get("gas")
        ):
        ...
        try:
            ...
            if results["stroom"]:
                results["stroom"] = sorted(
                    results["stroom"], key=lambda x: x.get("datum", "")
                )
            if results["gas"]:
                results["gas"] = sorted(
                    results["gas"], key=lambda x: x.get("datum", "")
                )
