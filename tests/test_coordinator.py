...
async def test_coordinator_successful_refresh(
    ...
    mock_stroom_vandaag.json.return_value = {
        "data": [{"datum": "2026-05-20 18:00:00", "prijsEE": 0.20}]
    }

    mock_stroom_morgen = AsyncMock()
    mock_stroom_morgen.status = 200
    mock_stroom_morgen.json.return_value = {
        "data": [{"datum": "2026-05-21 00:00:00", "prijsEE": 0.25}]
    }

    mock_gas_vandaag = AsyncMock()
    mock_gas_vandaag.status = 200
    mock_gas_vandaag.json.return_value = {
        "data": [{"datum": "2026-05-20 00:00:00", "prijsEE": 1.10}]
    }
...
@pytest.mark.asyncio
@patch("custom_components.enever_prijzen.coordinator.async_get_clientsession")
async def test_coordinator_api_limit_hibernation(
    mock_get_session, hass: HomeAssistant, mock_entry, mock_cache
):
...
async def test_coordinator_exception_handling(
    ...
    # FIX: Allow the session to be built, but make the network fetch .get() throw the timeout exception
    mock_session = MagicMock()
